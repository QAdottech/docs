...
Override device presets per-run via API using the `devicePresetId` parameter in the `applications` object. This allows you to test different device configurations without creating multiple test plans. See [Start Run API](/api-reference/start-run) for details.
