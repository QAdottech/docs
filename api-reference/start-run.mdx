...
- **`testPlanShortId`** (string): The **Test Plan Short ID** of the test plan to execute (this is the 6-character ID visible in test plan URLs)
- **`applications`** (object): Optional application overrides for testing specific environments and device configurations. Each application can specify:
  - An environment override: either a new environment (with `url` and `name`) or reference an existing environment (with `short_id`)
  - A device preset override: specify a `devicePresetId` (UUID) to use a different device configuration than the test plan default

<Tip>
  The key in the `applications` object is the **Application Short ID**
  (6-character alphanumeric, e.g., `gXeBl2`). Find it in **Settings ->
  Applications** (Short ID column or dropdown menu). See [Finding Application
  Short IDs](/api-reference/introduction#finding-application-short-ids).
</Tip>

- **`devicePresetId`** (string, UUID): Optional device preset UUID to override the default device preset for this application. Overrides the test plan's device preset setting for this specific run. See [Finding Device Preset UUIDs](/api-reference/introduction#finding-device-preset-uuids) for detailed instructions.
...
<Warning>
  Slack notification overrides currently do not work for projects with SSH
...
<Note>
  Use `short_id` to reference an **existing** environment (find it in
  Application Settings). Use `url` to create or update a **preview** environment
  dynamically (creates new preview environment, or updates existing one if it
  matches the same branch/repository).
</Note>
...
curl -X POST https://app.qa.tech/api/projects/YOUR_PROJECT_UUID/runs \
  ...
  -d '{
    ...
    "applications": {
      "gXeBl2": {
        "devicePresetId": "a1b2c3d4-e5f6-4789-a012-3456789abcde"
      }
    }
  }'
```

### Test Run with Environment and Device Preset Overrides
...
<Note>
  ...
  and examples: - [CI/CD Integration Overview](/configuration/ci-cd-integration)
  - Learn about integration modes - [GitHub
  Actions](/configuration/github-actions) - Official GitHub Action and examples
