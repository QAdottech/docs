...
<Warning>
  **Device Preset Limitation:** Environment overrides are supported via API, but
  device preset overrides are not. To test with different device configurations,
  create separate test plans with different device preset settings in the UI.
</Warning>
