...
If you have [Linear](/integrations/linear) or [Jira](/integrations/jira) connected, the assistant can search for issues by keyword, fetch a specific issue by key (e.g. `DEV-42`), list recent issues, and create new ones in your configured team or project. Jira users can also filter by issue type such as Epic, Bug, Story, or Task. If both integrations are connected, the assistant asks which tracker to use.
