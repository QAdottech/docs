...
**Via API:** Device preset overrides are supported using the `devicePresetId` field in the `applications` object. See [Start Run API](/api-reference/start-run) for details.
...
|---------|---------------|-------------------|
| Application Short ID | Settings -> Applications (column or dropdown menu) | Key in `applications` object |
| Environment Short ID | Settings -> Applications -> [App] -> Environments (above name) | `environment.short_id` field |
