---
title: MCP Server
description: 'Connect Claude Code, Claude Desktop, Cursor, Continue, or Goose to QA.tech so your AI assistant can list test cases, start runs, and read results.'
icon: 'plug'
---
...
| Client             | One-click install | Notes                                                    |
| ------------------ | ----------------- | -------------------------------------------------------- |
| Claude Code        | one-line `claude mcp add` command                  | Requires the [Claude Code CLI](https://claude.ai/code). |
| Cursor             | "Add to Cursor" button via [cursor.com/install-mcp](https://cursor.com/install-mcp) | Or paste the JSON snippet.               |
| Claude Desktop     | Paste JSON snippet                                 | Settings → Developer → Edit Config.      |
| Continue (VS Code) | Paste YAML snippet                                 | Edits `~/.continue/config.yaml`.         |
| Goose              | Paste YAML snippet                                 | Edits `~/.config/goose/config.yaml`.     |

Any other MCP-compatible client works too. Point it at the [endpoint](#endpoint-details) with a Bearer token.

## Pick Your Client

Most modern clients have a one-click or one-line install. Open the dashboard at **Settings → Integrations → MCP Server** to copy snippets with your API key pre-filled, or use the patterns below and replace `<API_KEY>` with the key from that page.

<Tabs>
  <Tab title="Claude Code">
    Run this in any project where you use Claude Code:

    ```bash
    claude mcp add --transport http qatech 'https://api.qa.tech/v1/mcp' --header 'Authorization: Bearer <API_KEY>'
    ```

    Claude Code reloads MCP servers automatically.
  </Tab>
  <Tab title="Cursor">
    The dashboard at **Settings → Integrations → MCP Server** shows an **Add to Cursor** button that opens Cursor and installs the server in one click with your API key pre-filled.

    To do it manually, add this to `~/.cursor/mcp.json`:
  ...
  <Tab title="Claude Desktop">
    Add this entry under `mcpServers` in your Claude Desktop config (**Settings → Developer → Edit Config**). Restart Claude Desktop after saving.
  ...
  <Tab title="Continue (VS Code)">
    Add this to `~/.continue/config.yaml`, then reload the Continue extension:
  ...
  <Tab title="Goose">
    Add this to `~/.config/goose/config.yaml`, then restart Goose:
