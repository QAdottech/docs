...
If **Auto-run on Merge Requests** is enabled, QA.tech can auto-trigger reviews when deployment data is ready.
...
For best results, include the preview deployment URL in the same comment. QA.tech parses URLs from MR comments and uses that URL as deployment context for the review run.

Example:

```text
@qa.tech please review this MR on the latest preview:
...
You can still trigger QA.tech test plans from GitLab CI/CD using the REST API.
