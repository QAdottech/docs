...
| **Test Creation** | You create tests in QA.tech UI | AI creates tests for new functionality |
...
- **Test preview/staging environments** - Override application URLs dynamically when testing Vercel, Netlify, or custom preview deployments. See [Preview Environments](/core-concepts/preview-environments).

- **Block deployments until tests pass** - Wait for test results before proceeding with your pipeline. The GitHub Action has built-in blocking support; other platforms can poll the run status API.
...
- **[Preview Environments](/core-concepts/preview-environments)** - Testing dynamic preview deployments
