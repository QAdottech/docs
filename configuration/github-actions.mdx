...
<Note>
Tests triggered via GitHub Actions automatically send [notifications](/core-concepts/notifications) to configured Slack channels when complete. When using the [direct API integration](#direct-api-integration), you can override notification destinations on a per-run basis to send results to specific Slack channels. See [Per-Run Notification Overrides](/core-concepts/notifications#per-run-notification-overrides) for details.
</Note>
...
### With Notification Overrides

Send notifications to specific Slack channels for this run:
...
This overrides the default project Slack channel and sends notifications to the specified channels instead. See the [Start Run API Reference](/api-reference/start-run#slack-notification-overrides) for complete details.
